...
*/
  ...
  display.println(txAR);
  display.println(txDEC);
  display.println(yaw);
...
void displayInfo()
{
  if(gps.location.isValid() && gps.location.isUpdated()){
    struct dms latDms = DegMinSec(gps.location.lat());
    struct dms lngDms = DegMinSec(gps.location.lng());

    latHH = latDms.degree;
    latMM = latDms.minutes;
    latSS = latDms.seconds;
  }
  ...
  if (SerialDebug)
  {
    ...
    if (gps.location.isValid())
    {
      ...
      Serial.print(F("/"));
      Serial.print(latDms.degree);
          Serial.print(("-"));
      Serial.print(latDms.minutes);
          Serial.print(("-"));
      Serial.print(latDms.seconds);
          Serial.print(F("/"));
    }
    else
    {
...
dms DegMinSec(double tot_val)
{
  volatile float minutes, seconds, remainder;
  volatile int degree, secs, mins;

  struct dms retVal;
  degree = (int)tot_val;
  remainder = tot_val - degree;
  minutes = 60 * remainder;
  mins = (int)minutes;
  remainder = minutes - mins;
